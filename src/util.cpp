...
const char * const BITCOIN_CONF_FILENAME = "illuminum.conf";
const char * const BITCOIN_PID_FILENAME = "illuminumd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "illuminum" is a composite category enabling all Illuminum-related debug output
            if(ptrCategory->count(string("illuminum"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\IlluminumCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\IlluminumCore
    // Mac: ~/Library/Application Support/IlluminumCore
    // Unix: ~/.illuminumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "IlluminumCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/IlluminumCore";
#else
    // Unix
    return pathRet / ".illuminumcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty illuminum.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override illuminum.conf
