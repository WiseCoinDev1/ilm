...
pkgdatadir = $(datadir)/illuminumcore
pkgincludedir = $(includedir)/illuminumcore
pkglibdir = $(libdir)/illuminumcore
pkglibexecdir = $(libexecdir)/illuminumcore
...
CONFIG_HEADER = $(top_builddir)/src/config/illuminum-config.h
CONFIG_CLEAN_FILES = libilluminumconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libilluminumconsensus.pc.in \
	...
	$(top_srcdir)/src/config/illuminum-config.h.in \
...
ACLOCAL = ${SHELL} "/home/genisis/work/clean /ilm/build-aux/missing" aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} "/home/genisis/work/clean /ilm/build-aux/missing" autoconf
AUTOHEADER = ${SHELL} "/home/genisis/work/clean /ilm/build-aux/missing" autoheader
AUTOMAKE = ${SHELL} "/home/genisis/work/clean /ilm/build-aux/missing" automake-1.15
AWK = mawk
BDB_CPPFLAGS = 
...
CXXFLAGS = -g -O2 -Wall -Wextra -Wformat -Wformat-security -Wno-unused-parameter -Wno-self-assign -Wno-implicit-fallthrough
CYGPATH_W = echo
DEFS = -DHAVE_CONFIG_H
DEPDIR = .deps
DLLTOOL = false
...
MAINT = 
MAKEINFO = ${SHELL} "/home/genisis/work/clean /ilm/build-aux/missing" makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = illuminumcore
PACKAGE_BUGREPORT = https://github.com/illuminumpay/illuminum/issues
PACKAGE_NAME = Illuminum Core
PACKAGE_STRING = Illuminum Core 1.0.1
PACKAGE_TARNAME = illuminumcore
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /home/genisis/work/clean /ilm
abs_srcdir = /home/genisis/work/clean /ilm
abs_top_builddir = /home/genisis/work/clean /ilm
abs_top_srcdir = /home/genisis/work/clean /ilm
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} '/home/genisis/work/clean /ilm/build-aux/install-sh'
...
pkgconfig_DATA = libilluminumconsensus.pc
BITCOIND_BIN = $(top_builddir)/src/illuminumd$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/illuminum-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/illuminum-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = Illuminum-Qt.app
OSX_DMG = Illuminum-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_illuminum_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_illuminum_coverage.info test_illuminum.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Illuminum-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/illuminum-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/illuminum-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/illuminum-config.h
$(top_srcdir)/src/config/illuminum-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/illuminum-config.h src/config/stamp-h1
libilluminumconsensus.pc: $(top_builddir)/config.status $(srcdir)/libilluminumconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/Illuminum-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Illuminum-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "Illuminum-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Illuminum-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_illuminum.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_illuminum -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_illuminum_filtered.info: test_illuminum.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_illuminum_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_illuminum_filtered.info
...
#test_illuminum_coverage.info: baseline_filtered_combined.info test_illuminum_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_illuminum_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_illuminum_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_illuminum_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_illuminum.coverage/.dirstamp:  test_illuminum_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_illuminum.coverage/.dirstamp total.coverage/.dirstamp

# Illuminum: disable the test until upstream switches from convoluted comparison tool
...
clean-local:
	rm -rf coverage_percent.txt test_illuminum.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
